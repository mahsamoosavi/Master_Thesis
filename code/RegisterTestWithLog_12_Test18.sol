...
contract RegisterTestWithLog_12_Test18 {
    ...
    enum Stages {DomainRegistration, AddDNSHash, AddTLSKey}
    Stages public stage = Stages.DomainRegistration;
    ...
    function nextStage() internal { // This is the current stage
        stage = Stages(uint(stage) + 1);
    }

    modifier transitionNext() // This modifier goes to the next stage after the function is done.
    {
        _;
        nextStage();
    }
...
//This function gets arguments from user and register the domain by adding a domian struct to Domians array
    ...
    function Register (bytes32 _DomainName)  payable atStage(Stages.DomainRegistration) transitionNext public returns(bool success){
        if (msg.value == reward)
        {
